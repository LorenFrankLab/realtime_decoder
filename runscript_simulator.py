...
def setup(config_path, numprocs):
    ...
    if rank in config['rank']['supervisor']:
        process = instantiation.create_main_process(
            comm, rank, config, simulation=True)
    elif rank in config['rank']['ripples']:
        process = instantiation.create_ripple_process(
            comm, rank, config, simulation=True)
    elif rank in config['rank']['encoders']:
        process = instantiation.create_encoder_process(
            comm, rank, config, simulation=True)
    elif rank in config['rank']['decoders']:
        process = instantiation.create_decoder_process(
            comm, rank, config, simulation=True)
